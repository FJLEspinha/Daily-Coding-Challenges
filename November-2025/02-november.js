...
worm was released, and you need to determine how many computers are infected 
...
For example, on:
    ...
    - Day 3: 8 total computers are infected. Then, apply the patch: 
      8 infected * 20% = 1.6 patched. 
      Round 1.6 up to 2. 
      8 computers infected - 2 patched = 6 total computers infected after day 3.

Return the number of total infected computers after the given amount of days have 
passed.
*/

function infected(days) {
    let totalInfectedComputers = 1
    for (let i = 1; i <= days; i++) {
        totalInfectedComputers *= 2
        if (i % 3 === 0) {
            const patched = (Math.ceil(totalInfectedComputers * 0.2))
            totalInfectedComputers -= patched
        }
    }

    return totalInfectedComputers
}
