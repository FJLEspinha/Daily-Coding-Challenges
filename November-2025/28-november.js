...
secret word using the following rules:
    ...
    - For each letter in the guess, replace it with a number according to how it matches 
      the secret word:
    ...
    - Exact matches ("2") are assigned first, then partial matches ("1") are assigned 
      from left to right for remaining letters.
    - If a letter occurs multiple times in the guess, it can only match as many times 
      as it appears in the secret word.

For example, given a secret word of "APPLE" and a guess of "POPPA", return "10201":

The first "P" is not in the correct location ("1"), 
the "O" isn't in the secret word ("0"), 
...
function compare(word, guess) {
    if (word.length !== guess.length) return 'Two words have not the same length'
    let secretWord = word.split('')
    let responseArray = new Array(word.length).fill(0)
    let indexToChange
    ...
    for (let i = 0; i < guess.length; i++) {
        if (secretWord.includes(guess[i])) {
            indexToChange = secretWord.indexOf(guess[i])
            responseArray[i] = 1
            secretWord[indexToChange] = '*'
        }
    }
