...
based on these constraints:
    ...
    - Return "Heat: X degrees Fahrenheit" if the current temperature 
      is below the target. With X being the number of degrees in 
      Fahrenheit to heat the room to reach the target, rounded to 
      1 decimal place.
    - Return "Cool: X degrees Fahrenheit" if the current temperature is above 
      the target. With X being the number of degrees in Fahrenheit to cool 
...
function adjustThermostat(currentF, targetC) {
    ...
    if (currentF === targetF) return 'Hold'
    
    return currentF < targetF
        ? `Heat: ${(targetF - currentF).toFixed(1)} degrees Fahrenheit`
        : `Cool: ${(currentF - targetF).toFixed(1)} degrees Fahrenheit`
}
