...
function sendMessage(route) {
    const velocityMessage = 300000
    const timeDelay = 0.5 * (route.length - 1)
    const finalTime = route.reduce((acc, current) => {
        const timeSeconds = current / velocityMessage
        return acc + timeSeconds
    }, 0) + timeDelay
    
    return parseFloat(finalTime.toFixed(4))
}
